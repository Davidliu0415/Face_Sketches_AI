...
ARCFACE_TRIPLET_TRIPLET_WEIGHT = 1.0
ARCFACE_TRIPLET_BATCH_HARD_WEIGHT = params.batch_hard_weight
ARCFACE_TRIPLET_FACE_CE_WEIGHT = 1.0
ARCFACE_TRIPLET_SKETCH_CE_WEIGHT = 1.0


def parse_args():
    parser = argparse.ArgumentParser(description="Run stage-3 Face-Sketch ArcFace + TripletLoss experiment.")
    parser.add_argument("--run-id", default="arcface_triplet_001")
    ...
    parser.add_argument("--output-dir", default=str(PROJECT_ROOT / "checkpoints" / "research_3"))
    ...
    parser.add_argument("--triplet-weight", type=float, default=ARCFACE_TRIPLET_TRIPLET_WEIGHT)
    parser.add_argument("--batch-hard-weight", type=float, default=ARCFACE_TRIPLET_BATCH_HARD_WEIGHT)
    parser.add_argument("--face-ce-weight", type=float, default=ARCFACE_TRIPLET_FACE_CE_WEIGHT)
    parser.add_argument("--sketch-ce-weight", type=float, default=ARCFACE_TRIPLET_SKETCH_CE_WEIGHT)
...
def apply_cli_overrides(args) -> None:
    global ARCFACE_TRIPLET_TRIPLET_WEIGHT
    global ARCFACE_TRIPLET_BATCH_HARD_WEIGHT
    global ARCFACE_TRIPLET_FACE_CE_WEIGHT
    global ARCFACE_TRIPLET_SKETCH_CE_WEIGHT

    ARCFACE_TRIPLET_TRIPLET_WEIGHT = args.triplet_weight
    ARCFACE_TRIPLET_BATCH_HARD_WEIGHT = args.batch_hard_weight
    ARCFACE_TRIPLET_FACE_CE_WEIGHT = args.face_ce_weight
    ARCFACE_TRIPLET_SKETCH_CE_WEIGHT = args.sketch_ce_weight
    ...
    torch.save(
        {
            ...
            "epoch": epoch,
            "val_loss": val_loss,
            "arcface_triplet_weights": {
                "triplet_weight": ARCFACE_TRIPLET_TRIPLET_WEIGHT,
                "batch_hard_weight": ARCFACE_TRIPLET_BATCH_HARD_WEIGHT,
                "face_ce_weight": ARCFACE_TRIPLET_FACE_CE_WEIGHT,
                "sketch_ce_weight": ARCFACE_TRIPLET_SKETCH_CE_WEIGHT,
            },
            "config": {
...
def write_arcface_triplet_results(path: Path, rows: list[dict]) -> None:
    fieldnames = [
        "run_id",
        "dataset",
        "train_samples",
...
def main():
    ...
    results_path = run_dir / "arcface_triplet_results.csv"
    ...
    print(
        "ArcFace + Triplet weights: "
        f"triplet={ARCFACE_TRIPLET_TRIPLET_WEIGHT}, "
        f"batch_hard={ARCFACE_TRIPLET_BATCH_HARD_WEIGHT}, "
        f"face_ce={ARCFACE_TRIPLET_FACE_CE_WEIGHT}, "
        f"sketch_ce={ARCFACE_TRIPLET_SKETCH_CE_WEIGHT}",
        flush=True,
    )
    ...
    if not args.skip_train:
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                if val_loader is not None:
                    val_stats = evaluate_triplet_epoch(
                        ...
                        triplet_weight=ARCFACE_TRIPLET_TRIPLET_WEIGHT,
                        batch_hard_weight=ARCFACE_TRIPLET_BATCH_HARD_WEIGHT,
                        face_ce_weight=ARCFACE_TRIPLET_FACE_CE_WEIGHT if val_labels_known else 0.0,
                        sketch_ce_weight=ARCFACE_TRIPLET_SKETCH_CE_WEIGHT if val_labels_known else 0.0,
                        log_interval=args.log_interval,
                        use_amp=args.amp,
                    )
                    ...
                    selection_value = -val_stats.loss
                    if val_retrieval_metrics:
                        selection_value = val_retrieval_metrics["mrr"]
        ...
        print(
            ...
            f"triplet_acc={val_stats.triplet_acc:.4f} "
            f"face_acc={val_stats.face_acc:.4f} "
            f"sketch_acc={val_stats.sketch_acc:.4f}",
            flush=True,
        )
    ...
    write_arcface_triplet_results(results_path, result_rows)
    ...
    print(f"arcface_triplet_results.csv: {results_path}", flush=True)
