...
BASELINE_BATCH_HARD_WEIGHT = 0.0
...
def parse_args():
    ...
    parser.add_argument("--triplet-weight", type=float, default=BASELINE_TRIPLET_WEIGHT)
    parser.add_argument("--batch-hard-weight", type=float, default=BASELINE_BATCH_HARD_WEIGHT)
    parser.add_argument("--face-ce-weight", type=float, default=BASELINE_FACE_CE_WEIGHT)
    parser.add_argument("--sketch-ce-weight", type=float, default=BASELINE_SKETCH_CE_WEIGHT)
...
def apply_cli_overrides(args) -> None:
    global BASELINE_TRIPLET_WEIGHT
    global BASELINE_BATCH_HARD_WEIGHT
    global BASELINE_FACE_CE_WEIGHT
    global BASELINE_SKETCH_CE_WEIGHT

    BASELINE_TRIPLET_WEIGHT = args.triplet_weight
    BASELINE_BATCH_HARD_WEIGHT = args.batch_hard_weight
    BASELINE_FACE_CE_WEIGHT = args.face_ce_weight
    BASELINE_SKETCH_CE_WEIGHT = args.sketch_ce_weight
    ...
    torch.save(
        {
            ...
            "baseline_weights": {
                ...
                "batch_hard_weight": BASELINE_BATCH_HARD_WEIGHT,
...
def main():
    ...
    print(
        ...
        f"batch_hard={BASELINE_BATCH_HARD_WEIGHT}, "
    ...
    if not args.skip_train:
        ...
        if not args.skip_val:
            ...
            val_dataset_identity_to_label = (
                identity_to_label if set(val_identity_to_label).issubset(identity_to_label) else val_identity_to_label
            )
